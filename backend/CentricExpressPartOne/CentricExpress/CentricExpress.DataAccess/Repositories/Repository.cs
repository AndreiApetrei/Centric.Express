...
namespace CentricExpress.DataAccess.Repositories
{
    public class Repository<T> : IRepository<T> where T: Aggregate
    {
        ...
        {
            return ExecuteWithLogging(() => AppDbContext.Set<T>().Find(id));
        }
        ...
        public void Remove(Guid id)
        {
            ...
            ExecuteWithLogging(() => this.AppDbContext.Set<T>().Remove(entity));
        }

        public void SaveChanges()
        {
            ExecuteWithLogging(() => this.AppDbContext.SaveChanges());
        }
